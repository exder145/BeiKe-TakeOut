    ...
    if request.method == 'POST':
        ...
        if userRole == 'ADMIN':
            ...
            try:
                ...
                print("Error: unable to use database!")
            sql = "SELECT * from ADMIN_USER where username = '{}' and password='{}'".format(username, password)
            cursor.execute(sql)
            db.commit()
            res = cursor.fetchall()
            num = 0
...
def shoppingCartPage():
    if request.method == 'GET':
        ...
        # 计算总价
        total_price = sum(float(item[3]) for item in res) + 5.00  # 加上配送费
        
        if len(res) != 0:
            msg = "done"
            return render_template('myOrder.html', username=username, result=res, messages=msg, total_price=total_price)
        else:
            msg = "none"
        ...
        if request.form["action"] == "加入购物车":
            ...
            sql1 = "INSERT INTO SHOPPINGCART (username,restaurant,dishname,price,img_res) VALUES ('{}','{}','{}',{},'{}')".format(
                username, restaurant, dishname, price, img_res)
            cursor.execute(sql1)
            db.commit()
            ...
            cursor.execute(sql)
            res = cursor.fetchall()
            total_price = sum(float(item[3]) for item in res) + 5.00
            ...
            # 插入订单
            sql = """INSERT INTO ORDER_COMMENT 
                    (orderID, username, restaurant, isFinished, cost, transactiontime) 
                    VALUES ('{}', '{}', '{}', 0, {}, NOW())""".format(
                order_id, username, restaurant, price)
            cursor.execute(sql)
            
            # 从购物车中删除已结算的商品
            sql = "DELETE FROM SHOPPINGCART WHERE username='{}' AND restaurant='{}' AND dishname='{}'".format(
                username, restaurant, dishname)
            cursor.execute(sql)
            db.commit()
    ...
    return render_template('myOrder.html', username=username)
